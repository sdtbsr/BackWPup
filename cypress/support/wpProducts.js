...
export class products{
    ...
    urlVerification(){
        cy.wait(500)
        cy.url().should('eq',backwpupUrl)
    }

    productVerification(){
        cy.wait(500)
        cy.get('[class="LicenceSection-boxHeadline"]').each((prod,index)=>{
    ...
    verifyTheTotalPriceForOneProduct(){
        ...
        cy.wait(500)
        cy.get('strong > .woocommerce-Price-amount > bdi').should('contain.text','199.00')
    }
    ...
    addSameProductTwoTimesToCart(){
        ...
        cy.wait(500)
        cy.get('button[type="submit"]').first().click()
        cy.contains(errorMessage.trim()).should('be.visible')
    }

    addMultipleProductsToCart(){
        
        for (let index = 1; index < 6; index++) {
            cy.get(':nth-child('+index+') > footer > form > .LicenceSection-button').click()
            cy.go('back')
